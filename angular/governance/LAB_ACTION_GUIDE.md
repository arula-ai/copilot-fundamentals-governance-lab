...
- Read `.github/instructions/angular.instructions.md` and each agent file in `.github/agents/`
- Inspect available tools (wrench icon) without changing settings
- Testing Mode (angular-testing): run `npm run lint` and `npm run test:coverage`; log assumptions in `docs/test-coverage.md`, we actually don't need to type `#runInTerminal` every time, each tool has a prompt associated with it that tells copilot when it can make use of it, the testing agent also tells it this
